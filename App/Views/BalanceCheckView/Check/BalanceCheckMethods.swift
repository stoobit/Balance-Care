...
extension BalanceCheckView {
    func scoreCheck() {
        Task {
            do {
                ...
                // Upload
                wrapper.state = await upload(wrapper.balanceCheck)
            } catch {
                print("ML:", error)
            }
        }
    ...
    func upload(_ balanceCheck: BalanceCheckModel) async -> BalanceUploadState {
        if shareBalanceData {
            do {
                try await upload.upload(balanceCheck)
                return .uploaded
            } catch {
                print(error)
            }
